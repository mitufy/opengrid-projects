...
vase_linewidth = 0.6;
//Multiple containers of same texture can be installed side by side as the pattern is designed to be complementary. Further customization available in Advanced Settings. 
vase_surface_texture = "cubes"; //["":None, "cubes":Cubes, "diamonds":Diagonal Ribs,"wave_ribs":Wave Ribs,"checkers":Checkers]
...
ocvase_top_profile = back(ocvase_small_rect_width / 2, rect([ocvase_small_rect_width, ocvase_small_rect_height], chamfer=[ocvase_small_rect_chamfer, ocvase_large_rect_chamfer, 0, 0], anchor=BACK));
...
module openconnect_vase_slot(add_nubs = "") {
  difference() {
    ...
    xrot(90) linear_extrude(ocslot_middle_to_bottom + ocslot_move_distance + ocslot_onramp_clearance) xflip_copy() polygon(ocvase_cut_side_profile);
