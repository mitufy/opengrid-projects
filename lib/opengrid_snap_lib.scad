...
function snap_text_cfg(
  add_custom_text = false,
  custom_text = "",
  custom_text_size = 4,
  text_depth = 0.4,
  thickness_text_mode = "Uncommon",
...
module snap_text(snapbody_cfg = [], snaptext_cfg = [], anchor = BOTTOM, spin = 0, orient = UP) {
  _snap_width = struct_val(snapbody_cfg, "snap_width", OG_SNAP_WIDTH);
  _snap_height = struct_val(snapbody_cfg, "snap_height", OG_SNAP_WIDTH);
  _snap_thickness = struct_val(snapbody_cfg, "snap_thickness", OG_STANDARD_THICKNESS);
  _add_custom_text = struct_val(snaptext_cfg, "add_custom_text", false);
  _custom_text = struct_val(snaptext_cfg, "custom_text", "");
  _custom_text_size = struct_val(snaptext_cfg, "custom_text_size", 4);
  _text_depth = struct_val(snaptext_cfg, "text_depth", 0.4);
  _thickness_text_mode = struct_val(snaptext_cfg, "thickness_text_mode", "Uncommon");
  _directional_arrow_depth = struct_val(snaptext_cfg, "directional_arrow_depth", 0.2);
  _add_blunt_text = struct_val(snaptext_cfg, "add_blunt_text", true);
  _add_arrow = struct_val(snaptext_cfg, "add_arrow", true);
  _add_thickness_text = _thickness_text_mode == "All" || (_thickness_text_mode == "Uncommon" && _snap_thickness != OG_LITE_BASIC_THICKNESS && _snap_thickness != OG_STANDARD_THICKNESS);

  attachable(anchor, spin, orient, size=[_snap_width, _snap_height, _text_depth]) {
    tag_scope() down(_text_depth / 2) {
        if (_add_blunt_text)
          back(_snap_height / 2 - 4.6) right(_snap_width / 2 - 4.6)
              linear_extrude(height=_text_depth)
                fill() text(OG_SNAP_THREADS_BLUNT_TEXT, size=4, anchor=str("center", CENTER), font=OG_SNAP_THREADS_BLUNT_TEXT_FONT);
        if (_add_thickness_text)
          fwd(_snap_height / 2 - 6) left(_snap_width / 2 - 4.2)
              linear_extrude(height=_text_depth)
                text(str(floor(_snap_thickness)), size=4, anchor=str("center", CENTER), font=OG_SNAP_TEXT_FONT);
        if (_add_custom_text)
          fwd(_snap_height / 2 - 3.2)
            linear_extrude(height=_text_depth)
              text(_custom_text, size=_custom_text_size, anchor=str("center", CENTER), font=OG_SNAP_TEXT_FONT);
        if (_add_arrow)
          up(_text_depth) back(_snap_height / 2 - 1.1) zrot(90)
                regular_prism(3, side=3.3, h=_directional_arrow_depth + EPS, chamfer1=_directional_arrow_depth, anchor=RIGHT + TOP);
      }
    children();
  }
}

function expanding_spring_cfg(
...
module expanding_spring(snapbody_cfg = [], bottom_type = "None", spring_cfg = [], snapcorner_cfg = [], snapcut_cfg = []) {
  _snap_thickness = struct_val(snapbody_cfg, "snap_thickness", OG_STANDARD_THICKNESS);
  ...
  up(_snap_thickness + EPS / 2) yrot(180) back(_snap_body_corner_inner_diagonal - gap_length - _spring_thickness + middle_gap_bottom_to_side)
        offset_sweep(gap_top_profile_rounded, height=_spring_face_chamfer + EPS, bottom=os_chamfer(width=-_spring_face_chamfer));

  right(_spring_thickness + _spring_gap) back(gap_length + _threads_negative_diameter / 2 + _spring_to_center_thickness) zrot(180)
  ...
  left(_spring_thickness + _spring_gap) back(gap_length + _threads_negative_diameter / 2 + _spring_to_center_thickness) zrot(180)
        offset_sweep(gap_top_profile_rounded, height=_snap_thickness + EPS, bottom=os_chamfer(width=-_spring_face_chamfer), top=os_chamfer(width=-_spring_face_chamfer));
}

module base_snap(snapbody_cfg = [], disable_features = [], snapcorner_cfg = [], snapnub_cfg = [], snapnotch_cfg = [], snapcut_cfg = [], snaptext_cfg = [], anchor = BOTTOM, spin = 0, orient = UP) {
  _snap_width = struct_val(snapbody_cfg, "snap_width", OG_SNAP_WIDTH);
  _snap_height = struct_val(snapbody_cfg, "snap_height", OG_SNAP_WIDTH);
  _snap_thickness = struct_val(snapbody_cfg, "snap_thickness", OG_STANDARD_THICKNESS);
  _snap_body_shape = struct_val(snapbody_cfg, "snap_body_shape", "Directional");
        ...
        cuboid([_snap_width, _snap_height, _snap_thickness], chamfer=OG_SNAP_CORNER_CHAMFER, edges="Z") {
          ...
          if (!in_list(list=disable_features, val="snap_text"))
            attach(BOTTOM, TOP, inside=true)
              snap_text(snapbody_cfg=snapbody_cfg, snaptext_cfg=struct_set(snaptext_cfg, ["add_arrow", _snap_body_shape == "Directional"]));
        }
    children();
  }
}
