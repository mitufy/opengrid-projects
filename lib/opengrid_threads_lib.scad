...
module snap_threads(threads_height = OG_STANDARD_THICKNESS, threads_cfg = [], text_cfg = [], anchor = BOTTOM, spin = 0, orient = UP) {
  ...
  _snap_threads_bottom_bevel =
    ...
    : threads_height >= OG_LITE_THICKNESS ? _snap_threads_bottom_bevel_lite
    : 0;
  ...
  attachable(anchor, spin, orient, d=_threads_diameter + _threads_clearance, h=threads_height) {
    tag_scope() diff() {
        ...
        if (struct_val(text_cfg, "text_depth") > 0)
          up(threads_height / 2 - EPS)
            tag("remove") snap_text(text_cfg=text_cfg, anchor=TOP);
      }
...
module expanding_threads(threads_height = OG_STANDARD_THICKNESS, threads_cfg = [], text_cfg = [], expand_cfg = [], anchor = BOTTOM, spin = 0, orient = UP) {
  ...
  _fold_angle = struct_val(_expand_cfg, "fold_angle");
  ...
  render() {
    attachable(anchor, spin, orient, d=_diameter, h=threads_height) {
      tag_scope() down(threads_height / 2) {
          if (_entry_height > 0)
            snap_threads(threads_height=_entry_height + EPS, text_cfg=_no_text_cfg, threads_cfg=struct_merge(_threads_cfg, _no_cutoff_cfg));
          for (a = [0:expand_segment_count - 1]) {
            ...
            zrot(-_fold_angle)
              partition(spread=-aseg_expansion_distance - EPS, cutpath="flat", $slop=aseg_expansion_distance / 2)
                zrot(_fold_angle) up(aseg_position) zrot(aseg_position * 120)
                      snap_threads(threads_height=expand_height_step + EPS, text_cfg=_no_text_cfg, threads_cfg=struct_merge(_threads_cfg, _no_cutoff_cfg));
          }
          if (_end_height > 0) {
            zrot(-_fold_angle)
              partition(spread=-expand_segment_count * expand_distance_step - EPS, cutpath="flat", $slop=expand_segment_count * expand_distance_step / 2)
                zrot(_fold_angle) up(_entry_height + transition_height) zrot((_entry_height + transition_height) * 120)
                      snap_threads(threads_height=max(_end_height, 0) + EPS, text_cfg=_no_text_cfg, threads_cfg=struct_merge(_threads_cfg, _no_cutoff_cfg));
          }
        }
      children();
    }
