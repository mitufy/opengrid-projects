        ...
        cuboid([shelf_back_thickness, top_shelf_back_height, shelf_width]) {
          ...
          if (shelf_type == "Slim")
            attach(LEFT, TOP, inside=true, spin=90)
              tag("remove") openconnect_slot_grid(horizontal_grids=horizontal_grids, vertical_grids=top_vertical_grids, tile_size=tile_size, slot_position=slot_position, slot_lock_distribution=slot_lock_distribution, slot_direction_flip=slot_direction_flip, excess_thickness=eps);
        }
