...
final_horizontal_grids = max(1, horizontal_grids);
shelf_depth = use_custom_depth ? custom_depth : OG_TILE_SIZE * depth_grids;
shelf_width = OG_TILE_SIZE * final_horizontal_grids;

shelf_linewidth = 0.42;
shelf_bottom_min_thickness = add_left_connector_holes || add_right_connector_holes ? 2.4 + shelf_linewidth * 3 : EPS;
shelf_bottom_max_thickness = OG_TILE_SIZE * top_vertical_grids - EPS;
final_shelf_bottom_thickness = min(max(shelf_bottom_thickness, shelf_bottom_min_thickness), shelf_bottom_max_thickness);
...
truss_depth = truss_beam_reach <= 0 ? 0 : max(0, (shelf_depth - shelf_back_thickness) * truss_beam_reach);
truss_height = truss_beam_reach <= 0 ? 0 : max(0, bottom_shelf_back_height - final_shelf_bottom_thickness);
truss_is_zero = truss_depth <= EPS || truss_height <= EPS || truss_thickness <= EPS;
truss_angle = truss_is_zero ? 0 : adj_opp_to_ang(truss_depth, truss_height);
truss_inner_depth = truss_is_zero ? 0 : truss_depth - ang_opp_to_hyp(truss_angle, truss_thickness);
truss_inner_height = truss_is_zero ? 0 : truss_height - ang_adj_to_hyp(truss_angle, truss_thickness);
//END shelf parameters

shelf_annotation_y = -bottom_shelf_back_height + final_shelf_bottom_thickness;
shelf_annotation_z = 0;
...
module emit_sturdy_shelf_annotations() {
  ...
  emit_dimension_annotation(
    ...
    basis="front_to_back_depth_from_depth_grids_or_custom_depth"
  );
  emit_dimension_annotation(
    id="horizontal_grids",
    ...
    basis="left_to_right_width_from_horizontal_grids"
  );
  emit_dimension_annotation(
    ...
    start=[0, top_vertical_grids * OG_TILE_SIZE, shelf_annotation_z],
    end=[shelf_back_thickness, top_vertical_grids * OG_TILE_SIZE, shelf_annotation_z],
  ...
  emit_dimension_annotation(
    ...
    start=[shelf_depth, 0, shelf_annotation_z],
  ...
  if (truss_beam_reach > EPS && truss_depth > EPS) {
    emit_dimension_annotation(
      ...
      axis="x",
      value=truss_depth,
      start=[shelf_back_thickness, shelf_annotation_y, shelf_annotation_z],
      end=[shelf_back_thickness + truss_depth, shelf_annotation_y, shelf_annotation_z],
    ...
    emit_dimension_annotation(
      ...
      start=[shelf_back_thickness, final_shelf_bottom_thickness, shelf_annotation_z],
      end=[shelf_back_thickness + truss_thickness, final_shelf_bottom_thickness, shelf_annotation_z],
  ...
  if (truss_strut_interval > EPS && truss_depth > truss_strut_interval * 2) {
    emit_dimension_annotation(
      ...
      start=[shelf_back_thickness + truss_strut_interval, final_shelf_bottom_thickness, shelf_annotation_z],
      end=[shelf_back_thickness + truss_strut_interval * 2, final_shelf_bottom_thickness, shelf_annotation_z],
  ...
  if (add_left_edge || add_right_edge) {
    emit_dimension_annotation(
      ...
      start=[shelf_depth, final_shelf_bottom_thickness, shelf_annotation_z],
      end=[shelf_depth, final_shelf_bottom_thickness, shelf_annotation_z + shelf_side_edge_depth],
  ...
  if (add_front_edge) {
    emit_dimension_annotation(
      ...
      start=[shelf_depth - shelf_front_edge_depth, final_shelf_bottom_thickness, shelf_annotation_z],
      end=[shelf_depth, final_shelf_bottom_thickness, shelf_annotation_z],
...
diff(remove="outer_rm")
  cuboid([shelf_depth, final_shelf_bottom_thickness, shelf_width], anchor=FRONT + LEFT + BOTTOM) {
    ...
    if (shelf_type != "Slim" && !truss_is_zero) {
