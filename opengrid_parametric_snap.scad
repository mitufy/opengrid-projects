      ...
      diff() {
        ...
        if (!disable_snap_threads) {
          tag("remove") down(eps / 2)
              up(reverse_threads_entryside ? snap_thickness + eps / 2 : 0) yrot(reverse_threads_entryside ? 180 : 0)
                  zrot(threads_compatiblity_angle + threads_offset_angle) generic_threaded_rod(d=multiconnect_threads_negative_diameter, l=snap_thickness + eps, pitch=3, profile=threads_profile, bevel1=0, bevel2=min(threads_bottom_bevel, snap_thickness), anchor=BOTTOM, blunt_start=false, internal=false);
        }
      }
    }
