...
/*[Shell Settings]*/
//Default wall thickness is set to products of 0.42, which is a common linewidth for 0.4mm nozzles.
...
/*[Container Settings]*/
container_solidwall_thickness = 2.52; //0.42
...
//Divide the drawer shell into several compartments. You will need to generate container for each manually.
...
//Compartment sizes in grid, separated by commas. This value is also the size of corresponding container.
...
container_divider_wall_height = 24.6;
container_divider_wall_thickness = 1.68; //0.42
container_divider_wall_fillet = 2;
container_divider_unit = "Width"; //["Width","Depth"]
//Unlike shell, container compartment's size isn't restrained by openGrid.
container_compartment_size_unit = 20;
//Compartment sizes separated by commas. grid_unit=20 and grids=2,3 means one 40mm and one 60mm compartment.
container_compartment_sizes = "2,3";
...
//Simple stoppers with no magnets involved. This can enabled together with side magnet holes.
add_stopper_holes = true;
stopper_clips_length = 8; //0.2

//Side magnets act as stoppers by holding the container when it's pulled out. Default thickness is 2mm but even thinner magnets allow for thinner wall thickness.
add_side_magnet_holes = true;
//If magnet is too thick for the wall, small "bulges" will be added.
shell_side_magnet_thickness = 2;
container_side_magnet_thickness = 2;
...
side_magnet_shell_edge_thickness = 0.84;
side_magnet_container_edge_thickness = 0.84;
...
shell_main_divide_tmp = slice(shell_main_divide_nums, 0, len(shell_main_divide_cumnums) - 1);
shell_main_divide_list = last(shell_main_divide_cumnums) == shell_main_divide_grids ? shell_main_divide_tmp : concat(shell_main_divide_tmp, [shell_main_divide_grids - last(shell_main_divide_cumnums)]);
shell_horizontal_compartments = add_shell_divider && shell_divideW ? shell_main_divide_list : [horizontal_grids];
echo(shell_main_divide_nums, shell_main_divide_cumnums, shell_horizontal_compartments);
          ...
          cuboid([shell_width, shell_height, shell_depth], anchor=BOTTOM) {
            ...
            if (add_side_magnet_holes) {
              ...
              ycopies(tile_size, vertical_grids)
                right(shell_slot_position == "Left" ? shell_ocslot_part_thickness : shell_thickness) left(shell_side_magnet_hole_thickness - eps * 2) attach(LEFT, FRONT, align=TOP, inset=side_magnet_shell_edge_thickness, inside=true)
              ...
              if (right_hc)
                attach(RIGHT + FRONT, RIGHT + FRONT, align=TOP, inside=true)
                  tag("keep") cuboid([shell_thickness, shell_height, 10.8]);
              ycopies(tile_size, vertical_grids)
                left(shell_slot_position == "Right" ? shell_ocslot_part_thickness : shell_thickness) right(shell_side_magnet_hole_thickness - eps * 2) attach(RIGHT, FRONT, align=TOP, inset=side_magnet_shell_edge_thickness, inside=true)
                      tag("rm_outer") teardrop(h=shell_side_magnet_hole_thickness, d=side_magnet_hole_diameter, cap_h=side_magnet_hole_diameter / 2 + 0.2);
            }
    ...
    if (shell_main_divide_cumnums[i] > 0 && shell_main_divide_cumnums[i] < shell_main_divide_grids) {
      ...
      tag_intersect(tag="keep", intersect="divider_mask", keep="divider_keep") {
        attach(mainsolidwall_anchor, mainsolidwall_anchor, align=mainwall_alignment, inset=main_compartment_inset, inside=true)
          tag_diff(tag=shell_divider_wall_type == "Honeycomb" ? "" : "divider_keep", remove="divider_remove") {
            cuboid([mainsolidwall_x, mainsolidwall_y, mainsolidwall_z]) {
              if (!shell_divideW && add_stopper_holes) {
                tag("divider_remove") line_copies(tile_size * (horizontal_grids - 1), 2)
        ...
        if (shell_divider_wall_type == "Honeycomb")
          tag_diff(tag="divider_mask", keep="inner_keep")
            attach(TOP, RIGHT, align=mainwall_alignment, inset=main_compartment_inset, inside=true, spin=mainhexwall_spin)
              tag("inner_keep") hex_panel([shell_hexpanel_depth, mainhexwall_length, shell_divider_wall_thickness], strut=honeycomb_strut_adj, spacing=honeycomb_unit_space_hyp, frame=shell_thickness) {
                  if (!shell_divideW && add_stopper_holes)
                    attach(RIGHT, TOP, inside=true)
                      tag("inner_keep") cuboid([shell_width, shell_divider_wall_thickness, 10.8]);
                  if (shell_divideW && add_side_magnet_holes)
                    attach(RIGHT, TOP, inside=true)
                      tag("inner_keep") cuboid([shell_height, shell_divider_wall_thickness, 10.8]);
                }
      }
    }
  }
    ...
    diff(remove="rm_outer", keep="mask kp_root") diff(remove="remove", keep="keep mask rm_outer kp_root")
        cuboid([container_width, container_height, container_depth], anchor=BOTTOM) {
          ...
          attach(BOTTOM, BOTTOM, align=FRONT, inset=container_bottom_wall_thickness, inside=true)
            tag(back_hc ? "remove" : "keep") cuboid([container_width - container_side_wall_thickness * 2, back_hc ? container_height - container_back_height_diff - container_bottom_wall_thickness * 2 : container_height - container_back_height_diff, container_back_wall_thickness + (back_hc ? eps : 0)], edges="Z", chamfer=container_inner_chamfer);

          // side walls
          container_side_wall_height = container_height - container_back_height_diff - container_bottom_wall_thickness - container_inner_chamfer - container_outer_chamfer;
          if (left_hc)
          ...
          attach(LEFT, LEFT, align=FRONT, inset=container_bottom_wall_thickness + container_inner_chamfer, inside=true)
            tag(left_hc ? "remove" : "keep") cuboid([container_side_wall_thickness + (left_hc ? eps : 0), container_side_wall_height - (left_hc ? container_side_wall_thickness : 0), container_depth - container_front_wall_thickness - container_back_wall_thickness]);
          attach(RIGHT, RIGHT, align=FRONT, inset=container_bottom_wall_thickness + container_inner_chamfer, inside=true)
            tag(right_hc ? "remove" : "keep") cuboid([container_side_wall_thickness + (right_hc ? eps : 0), container_side_wall_height - (right_hc ? container_side_wall_thickness : 0), container_depth - container_front_wall_thickness - container_back_wall_thickness]);
            ...
            xflip_copy() {
              ...
              if (container_side_wall_type == "Honeycomb")
                attach(LEFT + FRONT, LEFT + FRONT, align=BOTTOM, inside=true)
                  tag("keep") cuboid([container_side_wall_thickness, container_height - container_back_height_diff - container_outer_chamfer, shell_front_hex_offset + honeycomb_unit_space_hyp / 2]);
              ycopies(tile_size, vertical_grids) {
                attach(LEFT, FRONT, align=BOTTOM, inset=side_magnet_container_edge_thickness, inside=true, spin=90)
                ...
                if (hole_extrude_thickness > eps)
                  right(container_side_wall_thickness) attach(LEFT, FRONT, align=BOTTOM, inset=side_magnet_container_edge_thickness, inside=true, spin=90)
            ...
            if (back_hc)
              line_copies(back_magnet_grid_space[0], back_back_magnet_grid_count[0])
                attach(BOTTOM + FRONT, BOTTOM + FRONT, inside=true)
                  tag("keep") cuboid([back_magnet_hole_diameter + 4.2, container_height - container_back_height_diff, container_back_wall_thickness]);
...
module container_divider() {
  container_divideW = (container_divider_unit == "Width");
  container_divide_size = container_divideW ? container_width - container_side_wall_thickness * 2 : container_depth - container_front_wall_thickness - container_back_wall_thickness;
  container_main_divide_strs = str_split(str_strip(str_replace_char(container_compartment_sizes, " ", ","), ","), ",");
  container_main_divide_nums = [for (i = [0:len(container_main_divide_strs) - 1]) parse_num(container_main_divide_strs[i])];
  container_main_divide_cumnums = cumsum(container_main_divide_nums);
  ...
  container_mainsolidwall_y = min((container_height - container_back_height_diff - container_outer_chamfer), container_divider_wall_height + container_bottom_wall_thickness);
  // container_mainsolidwall_y = (container_height - container_back_height_diff) * container_divider_wall_height_scale;
  ...
  fillet_edges = container_divideW ? [FRONT + LEFT, FRONT + RIGHT] : [FRONT + TOP, FRONT + BOTTOM];
  for (i = [0:len(container_main_divide_cumnums) - 1]) {
    ...
    if (container_main_divide_cumnums[i] > 0 && container_main_compartment_inset < container_divide_size) {
      attach(container_mainsolidwall_anchor, container_mainsolidwall_anchor, align=container_mainwall_alignment, inset=container_main_compartment_inset, inside=true)
        tag("keep") cuboid([container_mainsolidwall_x, container_mainsolidwall_y, container_mainsolidwall_z]) {
            back(container_bottom_wall_thickness) edge_mask(fillet_edges)
...
module container_label_holder() {
  ...
  label_holder_height = label_height + 0.6 + label_side_clearance;
